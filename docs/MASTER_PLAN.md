...
| `ideas edit <id> [--rescore] [--vault <v>]`          | Open the idea section in `$EDITOR`. Strips `Score`/`ScoredAt`/`Rationale` on save so the idea re-enters the scoring queue. Title change recomputes the id (old id no longer resolves). `--rescore` calls `scout score` immediately after save. Records an `idea-edited` event.              |
