...
- **Revise** — plan is kept (intent is right) but needs changes (scope, content, approach). User provides feedback; Strategist redrafts; new revision goes back to `awaiting-review` and Slack re-surfaces it as a threaded reply under the original message so the conversation stays in one thread. Revision count tracked in plan metadata; no cap on revisions.
...
- **App metrics:** Umami (traffic, funnel, source attribution — self-hosted) + Supabase (signups, active users, conversions, revenue when live). Two signal kinds: `umami-metrics` (visitor/pageview aggregates, 7-day smoothed baseline) and `umami-events` (per-custom-event counts vs 7-day avg, suppressed per-app via `trackedEvents`). Signals are graded low/medium/high/critical; `critical` (site went dark) auto-triggers Strategist; medium/high land in the inbox for manual review.
...
**Phase 1 — Intake interview** (interactive). Jarvis walks you through a structured conversation about the business: origin story, problem, solution, market, traction, business model, competition, team, financials, risks, vision, blockers — 50+ sections in total. Intake is audience-blind: every applicable section is asked at standard depth regardless of who might later read a derived doc. Audience-tailored framing (pitch deck, investor update, mentor brief) is a presentation-time concern handled by downstream agents, not by skipping intake sections. Output is `vaults/<vault>/brains/<app>/docs/intake/content.txt`, registered as a cached doc with id `intake` in `docs.json`. Driven by [`prompts/strategist-intake.md`](../prompts/strategist-intake.md) + [`agents/intake.ts`](../agents/intake.ts). User controls termination: type `/skip` to skip a question, `/end` (or Ctrl-D) to wrap up early — the agent saves what it has and marks unanswered required sections as partial. Auto-skipped when stdin is not a TTY (CI, daemon, tests) and bypassable with `--skip-interview` for repos where you've already absorbed the equivalent docs.
...
| Analytics (apps)           | **Umami** (self-hosted) — deployed                                  | Live at `https://umami-self-seven.vercel.app` — canonical host for every onboarded app. Vercel free tier; Supabase Postgres (session pooler `:5432`, `uselibpqcompat=true&sslmode=require`); schema applied via `prisma migrate deploy`. Cookieless, GDPR-friendly; programmatic API for Analyst. Tracker script + `data-website-id` per app gets pasted into onboarding. |
...
4. **Revision loops** (§4). `revise <id> "<feedback>"` actually re-runs Strategist with the feedback in context — the plan flips awaiting-review → draft → awaiting-review with new content addressing the feedback. Slack re-surfaces each redraft as a threaded reply under the original message so the conversation stays grouped. Unbounded — revise as many times as needed. Amendment flow + escalate-within-plan (§12) defer to Phase 2 — they need daemon-driven background execution to pause/resume plans, which lands alongside Analyst's signal collectors.
...
- **Umami Tier 4+** — per-route breakdowns, funnel analysis, multi-week cyclical patterns, per-app websiteId-rotation support.
...
| `revise <id> "<feedback>"`                                                                                                                   | Send the plan back to `draft` with your feedback. Strategist redrafts and Slack re-surfaces in `awaiting-review` as a threaded reply under the original message, with revision count incremented. Distinct from `reject` — keeps the plan alive. No cap on revisions.                                                                                                                                                                                                       |
...
| `ideas edit <id> [--rescore] [--vault <v>]`          | **CLI:** Open the idea section in `$EDITOR`. Strips `Score`/`ScoredAt`/`Rationale` on save so the idea re-enters the scoring queue. Title change recomputes the id (old id no longer resolves). `--rescore` calls `scout score` immediately after save. Records an `idea-edited` event. **Slack:** `/jarvis ideas edit <query>` opens a modal editor with a body textarea (3000 char max) and "Rescore after save" checkbox; use the CLI for title/app/tags edits or bodies > 3000 chars. |
...
| `/jarvis logs [--lines N]`          | `yarn jarvis logs tail` — Slack returns an ephemeral snapshot of the last N lines (default 50, capped at 200) of today's daemon log. No streaming; one-shot. Truncates with a notice if the body would exceed Slack's ephemeral-message limit. |
...
| `/jarvis status`                    | `yarn jarvis status` — ephemeral snapshot: daemon liveness, plan counts by status, last 3 plan transitions, last agent call, calls today.                                                                                 |
