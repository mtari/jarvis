...
- **Umami metrics collector** — Vercel deploy + Supabase Postgres are live (see §17 deploy targets table). Remaining work: paste tracker `<script>` into each onboarded app, generate an API token into `jarvis-data/.env` as `UMAMI_API_URL` + `UMAMI_API_TOKEN`, and build the code-side metrics collector in `tools/umami.ts` so the Analyst sweep returns real data.
