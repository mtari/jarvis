...
166. **User wants a purpose-built showcase project.** Onboard `huntech.dev` — a bilingual (HU + EN) Hungarian dev community site combining three pillars: event aggregator (conferences + meetups + online events with any HU connection), OSS leaderboard (90-day public contributions; devs + HU-founded repos), and a monthly bilingual newsletter. Job board excluded. Stack: Astro + Vercel; data as YAML/JSON in repo. Public mono-repo `huntech-dev/huntech.dev` under a new GitHub org. Full scope locked in MASTER_PLAN.md §16 → Phase 5 Track B.
